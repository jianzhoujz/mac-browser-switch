...
### 3.4 **`setDefault` 后不要立刻刷新菜单**

`LSSetDefaultHandlerForURLScheme` 会触发 macOS 弹窗。这个弹窗有自己的
生命周期，期间应用菜单已经关掉了。等下次菜单打开（`menuWillOpen:`）
时再调 `currentDefault()` 即可。不要尝试同步 / 异步去回读结果再刷新
菜单，会和系统弹窗的状态打架。
